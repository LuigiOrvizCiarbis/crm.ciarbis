...
/**
 * Visor del documento adjunto de un contacto.
 *
...
export function DocumentViewerSheet({
  ...
  const load = useCallback(
    async (id: number, signal: AbortSignal) => {
      ...
      // La metadata primero: si el archivo no está, la cabecera igual puede
      // nombrar cuál es el que falta.
      try {
        setMeta(await fetchMediaAssetMeta(id))
      } catch (err) {
      ...
      try {
        const url = await fetchMediaAssetObjectUrl(id, signal)
        if (signal.aborted) {
  ...
  useEffect(() => {
    ...
    void load(assetId, controller.signal)
  ...
  // Sin metadata (falló /meta pero la descarga anduvo) el nombre igual tiene
  // que servir para guardar el archivo, así que lleva extensión.
  const fileName = meta?.name ?? "documento.pdf"
  const details = [
    formatFileSize(meta?.size),
    meta?.created_at ? format(new Date(meta.created_at), "d MMM yyyy", { locale: es }) : null,
    meta?.uploaded_by,
  ].filter(Boolean)
