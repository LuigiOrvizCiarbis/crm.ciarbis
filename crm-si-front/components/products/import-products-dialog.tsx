...
export function ImportProductsDialog({ open, onOpenChange, onImportComplete, productFields = [], resource = "products", title = "Importar catálogo", nativeTargets = native }: Props) {
  ...
  const send = async (action: "preview" | "queue") => {
    ...
    const form = new FormData(); form.append("file", file); form.append("mapping", JSON.stringify(payload(proposedFields))); form.append("mode", isContactImport ? "create" : mode); form.append("match_field", isContactImport ? "name" : matchField); form.append("preserve_empty", preserveEmpty ? "1" : "0"); form.append("original_filename", originalName); if (sheet) form.append("sheet_name", sheet)
    try {
