    ...
    <SidebarLayout>
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(280px,380px)_1fr]">
          ...
          <Card><CardHeader><CardTitle className="text-base">Detalle</CardTitle></CardHeader><CardContent>{!selected ? <p className="text-sm text-muted-foreground">Seleccioná un comentario.</p> : <div className="space-y-4"><div><p className="font-medium">@{selected.author_username || "usuario"}</p><p className="whitespace-pre-wrap text-sm">{selected.text}</p><p className="mt-2 text-xs text-muted-foreground">{selected.ad_title || (selected.media_id ? `Media ${selected.media_id}` : "Instagram")}</p></div><Textarea value={text} onChange={(event) => setText(event.target.value)} placeholder="Escribí una respuesta…" /><div className="flex flex-wrap gap-2"><Button disabled={!text.trim()} onClick={() => void action("/reply", { text })}>Responder públicamente</Button><Button variant="secondary" disabled={!text.trim() || !!selected.private_replied_at} onClick={() => void action("/private-reply", { text })}>Enviar por privado</Button><Button variant="outline" onClick={() => void action(selected.visibility === "hidden" ? "/unhide" : "/hide")}>{selected.visibility === "hidden" ? "Mostrar" : "Ocultar"}</Button><Button variant="destructive" onClick={() => { if (window.confirm("¿Eliminar este comentario de Instagram?")) void action("/delete") }}>Eliminar</Button></div>{selected.private_reply_deadline && <p className="text-xs text-muted-foreground">Respuesta privada disponible hasta {new Date(selected.private_reply_deadline).toLocaleString()}</p>}</div>}</CardContent></Card>
        </div>
      </div>
    </SidebarLayout>
  )
