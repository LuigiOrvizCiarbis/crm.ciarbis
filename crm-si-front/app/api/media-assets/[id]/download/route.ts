...
/**
 * Descarga del PDF de un contacto.
 *
 ...
 * archivo en memoria del servidor de Next.
 */
export async function GET(req: NextRequest, { params }: RouteContext) {
  ...
  for (const base of bases) {
    try {
      const res = await fetch(`${base}/api/media-assets/${id}/download${download}`, {
        headers: { Authorization: authHeader, Accept: "application/pdf" },
        // Un PDF escaneado de varias decenas de MB por una conexión lenta no
        // entra en el timeout de 15s que usa el proxy genérico.
